...
class SplineMeshData:
    """Raw mesh data generated from smooth lamella profiles."""
...
class SplineMeshBuilder:
    """Creates a printable mesh from smooth Lamella objects."""
    ...
    def build(
        ...
        for lamella in lamellas:
            SplineMeshBuilder._add_lamella(
                lamella=lamella,
                vertices=vertices,
                faces=faces,
            )
        ...
        if base_plate_enabled:
            SplineMeshBuilder._add_base_plate(
                lamellas=lamellas,
    ...
    def _add_lamella(
        ...
        for y_position, depth_value in zip(
            lamella.y_positions_mm,
            lamella.depth_values_mm,
        ):
            y = float(y_position)
            depth = max(0.0, float(depth_value))
        ...
        for row in range(lamella.point_count - 1):
            ...
            # Smooth upper profile
            faces.append(
                [left_top, right_top, next_left_top]
            )
            faces.append(
    ...
    def _add_base_plate(
        ...
        minimum_x = min(
            float(lamella.left_x_mm)
            for lamella in lamellas
        ) - margin

        maximum_x = max(
            float(lamella.right_x_mm)
            for lamella in lamellas
        ) + margin
        ...
        vertices.extend(
            [
                ...
                [minimum_x, minimum_y, top_z],
                [maximum_x, minimum_y, top_z],
                [maximum_x, maximum_y, top_z],
                [minimum_x, maximum_y, top_z],
            ]
        )
